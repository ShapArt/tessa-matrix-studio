...
### [2 · УСТАНОВИТЬ TESSA MATRIX STUDIO](https://github.com/ShapArt/tessa-matrix-studio/releases/latest/download/tessa-matrix-studio.user.js)

[РЕЛИЗ](https://github.com/ShapArt/tessa-matrix-studio/releases/latest) · [КОД](https://github.com/ShapArt/tessa-matrix-studio/blob/main/src/core.user.js) · [ISSUES](https://github.com/ShapArt/tessa-matrix-studio/issues/new/choose)
...
- массовые **добавление / изменение / удаление** строк через привычный Excel;
...
В **1.14.2** Full UAT и release gate синхронизированы с живой TEST-проверкой: 37/37 обязательных сценариев прошли без FAIL/NOT RUN. Улучшены переносы между карточками одного шаблона, Preview больших изменений, сопоставление сотрудников по ФИО/RoleID и picker для нескольких должностей. Должность остаётся уточняющей подсказкой: уникальное ФИО без должности допустимо, а неоднозначные тёзки блокируются fail-closed.
...
<div align="center">
  <img src="docs/assets/studio-start-v1.14.1.jpg" alt="Стартовая панель TESSA Matrix Studio v1.14.1" width="420">
  <br><sub>Актуальная панель Studio v1.14.1 внутри карточки матрицы TESSA.</sub>
</div>
...
Полный указатель для пользователей, разработчиков, QA, ИТ и ИБ находится в [центре документации](docs/README.md).

| Задача | Документ |
|---|---|
| Установить и начать работу | [Быстрый старт](#быстрый-старт) |
| Проверить справочники, объединение и запись | [Актуальный UAT](docs/UAT-v1.17.0.md) |
...
Обновление справочников в выбранном Excel сохраняет рабочие правки, стили и пользовательские листы. Объединение с актуальной TESSA показывает конфликты до записи. [Актуальные сценарии UAT](docs/UAT-v1.17.0.md).
...
**Серверный `LeftOperandExtractor is null` остаётся открытым live-блокером (#57).** В 1.11.9 добавлен узкий повтор через `CardNewMode.Valid`, но свежий live-прогон 1.11.9 снова получил `duplicate-interval-extractor`. Studio не трактует такой ответ как «дублей нет»: серверный `ValidateDuplicate` не обходится, а Store не запускается без успешной проверки.
...
Перед релизом выполните [компактный полный набор UAT](docs/UAT-COMPACT-ALL-CASES.md): правильные и ошибочные значения в одном файле, все рабочие поля, новые и пустые строки, защита от формул и проверка результата записи.
...
<div align="center">
  <img src="docs/assets/excel-matrix-v1.14.1.jpg" alt="Реальная Excel-выгрузка TESSA Matrix Studio v1.14.1" width="760">
  <br><sub>Реальная рабочая выгрузка матрицы, без нарисованных макетов.</sub>
</div>
...
| Заполнили новую строку или изменили вставленную копию | **ДОБАВИТЬ** | создастся новая строка |
| Вставили полностью идентичную существующей строку | **БЕЗ ИЗМЕНЕНИЙ** | повторная запись точного дубля не выполняется |
...
| Очистили все рабочие ячейки существующей строки | **УДАЛИТЬ** | строка будет удалена; скрытые ID очищать не требуется |
| Оставили пустые строки, затем заполнили новую | **ДОБАВИТЬ** | пустые промежутки игнорируются |
...
<div align="center">
  <img src="docs/assets/studio-preview-v1.14.1.jpg" alt="Реальный preview изменений TESSA Matrix Studio v1.14.1" width="520">
  <br><sub>Preview актуальной версии: каждая операция видна до записи в TESSA.</sub>
</div>
...
<div align="center">
  <img src="docs/assets/studio-apply-v1.14.1.jpg" alt="Применение проверенного плана TESSA Matrix Studio v1.14.1" width="520">
  <br><sub>Перед Apply видны итоговые счётчики и только тот набор операций, который будет записан.</sub>
</div>

## 5. Скачать Excel-отчёт изменений

После Preview можно скачать отдельный report-only Excel-файл с изменениями. Он предназначен для просмотра и контроля и **не принимается обратно в Studio для Apply**.

В отчёте действия подписаны по-русски: **Изменена / Добавлена / Удалена / Пропущена**. Для UPDATE показаны изменённые поля, а для ADD и DELETE разворачиваются заполненные бизнес-поля строки. Основные колонки — **Действие, Строка Excel, Строка TESSA, Поле, Было, Стало**.

<div align="center">
  <img src="docs/assets/changes-report-v1.14.1.jpg" alt="Excel-отчёт было-стало TESSA Matrix Studio v1.14.1" width="760">
  <br><sub>Отдельный отчёт «было → стало» для проверки и разбора выполненных изменений.</sub>
</div>
