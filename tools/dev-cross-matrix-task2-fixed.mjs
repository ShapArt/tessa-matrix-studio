...
const oldStart = `  function buildPlan(workbook, structure, snapshot) {
...
`;
if (!code.includes(oldStart)) throw new Error('buildPlan start marker not found');
const newStart = `${indent(foreignDesiredRow)}\n\n${indent(buildCrossMatrixReplacementPlan)}\n\n  function buildPlan(workbook, structure, snapshot, matrixInfo = null) {
    const columnMap = buildColumnMap(workbook, structure);
    const desired = workbookRowsToDesired(workbook, columnMap);
...
const oldWarnings = `    const warnings = [...columnMap.warnings, ...(built.warnings || [])];`;
if (!code.includes(oldWarnings)) throw new Error('warnings marker not found');
code = code.replace(oldWarnings, `    const rawWarnings = [...columnMap.warnings, ...(built.warnings || [])];
...
const oldMetadata = `      counts: countActions(actions, skippedRows),
    };`;
if (!code.includes(oldMetadata)) throw new Error('plan metadata marker not found');
code = code.replace(oldMetadata, `      counts: countActions(actions, skippedRows),
      workbookContext,
...
const analyzeMarker = `    const plan = buildPlan(workbook, structure, snapshot);`;
if (!code.includes(analyzeMarker)) throw new Error('analyze buildPlan marker not found');
code = code.replace(analyzeMarker, `    const plan = buildPlan(workbook, structure, snapshot, bridge.matrixInfo());`);

const exportMarker = `    readXlsxArrayBuffer, parseSheetXml, buildColumnMap, workbookRowsToDesired, buildPlan,`;
if (!code.includes(exportMarker)) throw new Error('build exports marker not found');
code = code.replace(exportMarker, `    readXlsxArrayBuffer, parseSheetXml, buildColumnMap, workbookRowsToDesired, foreignDesiredRow, buildCrossMatrixReplacementPlan, buildPlan,`);

fs.writeFileSync(userPath, code);

const packagePath = 'package.json';
const pkg = JSON.parse(fs.readFileSync(packagePath, 'utf8'));
const testMarker = 'node tests/cross-matrix-context.mjs';
if (!pkg.scripts.test.includes('node tests/cross-matrix-replace.mjs')) {
  if (!pkg.scripts.test.includes(testMarker)) throw new Error('package context test marker not found');
  pkg.scripts.test = pkg.scripts.test.replace(testMarker, `${testMarker} && node tests/cross-matrix-replace.mjs && node tests/cross-matrix-schema-drift.mjs`);
}
