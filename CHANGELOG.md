...
- Активное дерево переведено на единственный канонический `src/`: удалены одноразовые hotfix/recovery scripts, устаревшие workflow и сгенерированный userscript. История сохранена в Git и release tags.
- Пять действующих workflow и восемь инструментов зафиксированы автоматической hygiene-проверкой; Markdown-ссылки проверяются в regression suite.
- Список тестов вынесен из длинной команды `package.json` в `tests/suite.json`, добавлены центр документации, руководство участника и регламент сопровождения репозитория.
- Production и UAT артефакты остались байт-в-байт идентичны сертифицированной версии 1.17.1.

## 1.17.1 — 2026-09-25

- Full UAT формирует единый support ZIP из изолированного тестового контекста и проверяет его manifest, SHA-256 и обязательные Excel-файлы без зависимости от выбранного в UI файла.
- Временные ADD-строки Full UAT нормализуют исторический вид документа по актуальному `MtxRouteTemplate.CardTypeID`; клиентская и серверная проверки области документа не ослаблены.
- Конфликтный merge-сценарий выбирает справочник минимум с тремя актуальными значениями, поэтому обязательная проверка больше не уходит в случайный `NOT RUN`.
- Обновление справочников, объединение с актуальной TESSA, диагностика и тестовые действия выведены в плоский видимый список. Full UAT получает отдельный видимый host в UAT-сборке.
- При смене файла и `pagehide` освобождаются удерживаемые OPC-части XLSX. Регрессии проверяют изолированный support bundle, область видов документов, реальные клики, один browser download и десять циклов удерживаемой памяти.
- Production скрывает служебные проверки под сворачиваемым разделом, не содержит Full UAT host/runner/test exports и ограничен тремя точными TESSA origins. UAT остаётся отдельным exact CI-артефактом.
- Live UAT: `PASS 51 / FAIL 0 / NOT RUN 0`, 21/21 записей, 32/32 cleanup-обязательств и `restoreStatus=VERIFIED`.

## 1.17.0 — 2026-09-25

- Новый `TESSA_MATRIX_ROUNDTRIP_V7` хранит одинаковые справочники одним физическим диапазоном и использует `sharedStrings.xml`; чтение V1–V6 сохранено.
- Preview, выбранный Excel, актуальная матрица, изменения и очищенные отчёты объединены в `TESSA_MATRIX_SUPPORT_BUNDLE_V1` с SHA-256 manifest. Отдельные кнопки скачивания заменены одной кнопкой «Скачать пакет».
- Фильтры Preview проверяются реальными кликами и клавиатурой в Chromium. Browser UAT подтверждает download event, ZIP/XLSX, SHA-256 и удержание памяти после десяти циклов.
- Runtime, Full UAT и interval fallback разделены; единый детерминированный builder собирает production и exact UAT. Production не публикует тестовые функции в `window`.
- Добавлены 30-минутный изолированный кэш справочников, единый формат performance telemetry, сброс кэша после записи и фильтрация видов документов по CardTypeID шаблона.
- Бизнес-значения в основных карточках Preview выводятся DOM-узлами через `textContent`. Диагностика пакета строится по allowlist и не включает сырые ответы TESSA.

## 1.14.2 — 2026-09-18

- Full UAT доведён до детерминированного release gate: SET→CLEAR больше не уходит в случайный NOT_RUN, copied-row identity сценарий предварительно исключает случайные бизнес-дубли, а итоговый verdict требует явных evidence для всех действий.
- Apply/Reconcile в Full UAT теперь подтверждаются реальными операциями: Apply — завершёнными write/read-back циклами, Reconcile — штатным `runReconciliationRead` по mutation receipt. Живой TEST-прогон: **PASSED, 37 PASS / 0 FAIL / 0 NOT RUN**, seed **1664950438**.
- Встроен production-shadow профиль для объёма PROD-класса: 100 009 справочных записей и synthetic same-template transfer 488→103, чтобы CI не зависел от маленьких тестовых справочников.
- Сотрудники разрешаются по стабильной identity/ФИО; должность служит уточняющей подсказкой. Уникальное ФИО без должности остаётся валидным, тёзки без достаточного уточнения блокируются fail-closed, одна должность без ФИО не угадывается.
- Исправлены сотрудники с несколькими должностями и native caption с `;`: picker формирует одно безопасное Excel-значение, не дублирует должность в заголовке и сохраняет обратное разрешение в исходный RoleID/RoleTypeID.
- Preview для больших переносов получил явные счётчики «не будет применено»/ошибок, bounded Resolution Center и фильтрацию через основные счётчики. Неполный cross-matrix replacement по-прежнему атомарно блокируется; частичное удаление/добавление не выполняется.
- Release-композиция синхронизирована с exact v1.14.2 UAT-кандидатом; публикация требует тот же набор transforms, regression tests, native live evidence, SHA-256, provenance attestation и проверку публичного `releases/latest`.
...
- Исправлен смешанный импорт: новые строки и копии больше не отключают удаление отсутствующих исходных ID.
- Очистка всех рабочих ячеек строки означает DELETE; пустые промежутки не мешают ADD ниже них. Обновление Excel-схемы сохраняет оба вида удаления вместе с добавлениями.
- Копия ниже исходной области Excel не подменяет удалённую строку даже при неизменном общем количестве строк.
- Убраны запреты DELETE по количеству и проценту. Проверки матрицы, исходных версий, справочников и серверной валидации сохраняются.
- Обычная кнопка Apply автоматически сверяет результат с матрицей; расхождения и невозможность проверки дают частичный статус и число подтверждённых операций.
- Добавлены обезличенные регрессии по сценарию 24 исходных строк: 3 UPDATE, 1 ADD, 2 DELETE, 20 NOOP. На исходной версии тест воспроизводит дефект.
- Ошибка серверного ValidateDuplicate `LeftOperandExtractor is null` не считается исправленной; существующий проверяемый fallback сохранён без отключения серверной проверки.
...
- Исправлена привязка Excel-справочников для функций матрицы в реальной TESSA: GUID `FunctionType.ID` больше не трактуется как `RoleTypeID` и не сваливает все функции в один общий `MtxRoles`.
- Для каждой функции с GUID/кастомным типом создаётся отдельный каталог ролей. Типы ролей, уже используемые именно этой функцией, идут первыми; Personal используется как безопасный следующий вариант, при этом Department/Group/Static и другие допустимые роли не удаляются.
- Существующие значения сохраняются и разрешаются по точной паре `RoleID|RoleTypeID`, включая legacy и корпоративные типы ролей (например `RoleTypeID=9`). Установки, где `FunctionType.ID` действительно числовой RoleType, сохраняют строгую фильтрацию.
- Версия проекции справочников поднята до 4, поэтому старый локальный кэш с общим `MtxRoles` не переживает обновление.
- Добавлены регрессии для реального GUID типа функции, независимых function-пикеров и маршрутизации ссылочных критериев по `View + RefSection`; строковые/числовые поля не получают ложные выпадающие списки.
- Apply/Store/Delete/ValidateDuplicate и fail-closed обработка `LeftOperandExtractor is null` не менялись.
...
- Отдельное действие применяет выбранный Preview с обычным подтверждением и сразу сверяет результат записи. Ошибка `LeftOperandExtractor` не обходится и остаётся открытой. Актуальные границы проверки описаны в [UAT 1.17](docs/UAT-v1.17.0.md).
