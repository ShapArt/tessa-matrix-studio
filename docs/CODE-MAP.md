...
Канонический рабочий код находится в [`src/core.user.js`](../src/core.user.js). Устанавливаемый userscript создаётся детерминированным builder без внешних runtime-зависимостей. Номера секций в комментариях помогают найти нужную часть; имена функций можно искать через `rg` или поиск редактора.
...
[Архитектура](ARCHITECTURE.md) описывает API и identity. [Runbook](PRODUCTION-RUNBOOK.md) — эксплуатацию, ошибки и выпуск. [UAT 1.17](UAT-v1.17.0.md) — пользовательские проверки и границы автоматического подтверждения.
