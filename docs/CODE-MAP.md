...
`TessaBridge.prepareRowCardForServer` подготавливает отдельную карточку перед `validateDuplicate` и `storeRowCard`: удаляет только известные пустые виртуальные секции штатного редактора. Непустые буферы отклоняются до запроса. [Причина и проверки](NATIVE-ROW-PAYLOAD.md).
...
## Диагностика интервалов (1.9.49)

`collectIntervalDiagnostics` сравнивает максимум четыре образца через исходный `TessaBridge.validateDuplicate`. Локальный facade CardService перехватывает только запрос проверки дубликатов и копирует хранилище до отправки; общий сервис не заменяется. Контрольная карточка перечитывается, затем клонируется; семантика копии проверяется до отправки. Код не вызывает Apply/preflight/Store/Delete и не изменяет разрешения плана.

`runIntervalDiagnostics` выполняется только по клику, блокирует конкурирующие действия Studio и проверяет текущую матрицу/шаблон/файл перед сетевыми этапами. Отдельный `lastIntervalDiagnostics` очищается при новом файле, контексте и применении. Обычный `downloadJson` сохраняет прежний фильтр приватных полей; только этот добровольный технический отчёт передаёт `replacer=null`, чтобы сохранить request.info.card. Регрессия: `tests/interval-diagnostics.mjs`.

## Общая диагностика (1.9.51)

- `collectStudioDiagnostics`: независимые этапы с pass/fail/not-run, ограниченный захват бизнес-данных и отдельный bridge с allowlist запросов. Не возвращает план, пригодный для Apply.
- Внутренние `guard`, `run`, `rpc`, `capture`: контроль контекста, фиксация результата этапа, ограничение запросов/таймаута и сохранение доступных данных. Ожидаемый отказ дубликата не равен транспортной ошибке.
- `makeStudioDiagnosticPackage`: локальный ZIP из уже собранных данных. Не обращается к TESSA. `result.source` — только ссылки в памяти для проверки актуальности; в ZIP не сериализуется.
- `runStudioDiagnostics`: блокировка повторного запуска, проверка файла/матрицы/шаблона до запросов и перед скачиванием, кэш результата; отмена сохраняет частичный пакет только при прежнем контексте.
